...
def my_model():
    ...
    model_training.add(Conv2D(32, kernel_size=(5, 5), input_shape=(imageDimensions[0], imageDimensions[1], 1), activation='relu'))
    model_training.add(MaxPooling2D(pool_size=(2, 2)))

    model_training.add(Conv2D(64, kernel_size=(3, 3), activation='relu'))
    model_training.add(MaxPooling2D(pool_size=(2, 2)))
    model_training.add(Dropout(0.5))

    model_training.add(Flatten())
    model_training.add(Dense(128, activation='relu'))
    model_training.add(Dense(noOfClasses, activation='softmax'))

    # COMPILE MODEL
    model_training.compile(Adam(learning_rate=0.001), loss='categorical_crossentropy', metrics=['accuracy'])
...
history = model.fit(dataGen.flow(X_train, y_train, batch_size=batch_size_val), steps_per_epoch=len(X_train) // batch_size_val, epochs=epochs_val, validation_data=(X_validation, y_validation))
